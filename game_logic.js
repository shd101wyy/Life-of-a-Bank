...
var drawBranchEmployee;
var drawHireInterface; 
var drawHireEmployee;
// scenes
...
drawHireInterface = function(branch_object) {
  ...
  var title = new Label("Hire Employee");
  title.x = 60;
  title.y = 40;
  title.font = "40px myFirstFont";
  Hire_Scene.addChild(title);
  ...
  for (var i = 0; i < employeesAvailable.length ; i++) {
  drawHireEmployee(branch_object, employeesAvailable[i], 60, 150 + i * 150);
 }
 ...
 /**
  * draw employees for hire
  */
drawHireEmployee = function(branch_object, employee_object, x, y) {
 var level = new Label("Level: " + employee_object.stars);
 level.x = 60;
 level.y = y + 30;
 level.font = "35px myFirstFont";

 var salary = new Label("Salary: " + parseInt(employee_object.salary));
 salary.x = 60;
 salary.y = y + 60;
 salary.font = "35px myFirstFont";
 ...
 employee_picture.x = x + 300;
 employee_picture.y = y;
 
 var hire_employee = function() {
  branch_object.clickHire(employee_object);
  game.removeScene(Hire_Scene);
  drawSpecificBranchInformation(branch_object);
 }
 level.addEventListener("touchstart", hire_employee);
 salary.addEventListener("touchstart", hire_employee);
 employee_picture.addEventListener("touchstart", hire_employee);
 
 Hire_Scene.addChild(level);
 Hire_Scene.addChild(salary);
 Hire_Scene.addChild(employee_picture);
...
drawHomeMenuScene = function() {
 ...
 var current_month_label = new Label("Months in business: " + Player.month);
 current_month_label.x = 60;
 current_month_label.y = game_height - 300;
 current_month_label.font = "25px myFirstFont";
...
drawBranchEmployee = function(branch_object, employee_object, x, y) {
 var level = new Label("Level: " + employee_object.stars);
 level.x = 60;
 level.y = y + 60;
 ...
 fire_button.addEventListener("touchstart", function() { // fire
  ...
  //game.remove(Branch_Information_Scene);
 ...
 */
 ...
 var sellBranch = function() { // redraw scene after add branch
  ...
  game.removeScene(Branch_Information_Scene); //remove current scene
  drawBranchesScene()
 }
 ...
 for (var i = start_i; i < start_i + 4; i++) {
  ...
  drawBranchEmployee(branch_object, branch_object.employees[i], 60, 200 + (i - start_i) * 100);
 }
